...
void addPiece(SDL_Renderer* renderer, int shapeNumber, int orientation, coordinates coords, int **grid)
{
...
void addPiece2(SDL_Renderer* renderer, int shapeNumber, int orientation, coordinates coords, int **grid)
{
    coordinates startingPoint = calculateStartingPoint();
...
void renderPiece(SDL_Renderer* renderer, SDL_Texture* backgroundTexture, SDL_Texture* buffer, coordinates boardCoords, int shapeNumber, int orientation, int **grid) 
{
...
{
    int **newGrid = new int*[boardWidth];
    for (int i=0; i<boardWidth; i++)
    {
        newGrid[i] = new int[boardHeight];
        for (int j=0; j<boardHeight; j++)
        {
    ...
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 4; j++)
        {
            incomingPiece[i][j] = pieces[shapeNumber][orientation][i][j];
            if (incomingPiece[i][j] == 1)
                newGrid[boardCoords.x + i + padding][boardCoords.y + j + padding] += 1 * (shapeNumber + 1);
        }
    }
...
bool nextMove(SDL_Renderer* renderer, SDL_Texture* backgroundTexture, SDL_Texture* buffer, int **grid, coordinates boardCoords, int shapeNumber, int orientation)
{
    int **newGrid = new int*[boardWidth];
    for (int i=0; i<boardWidth; i++)
    {
    ...
    newGrid = updateGrid(newGrid, boardCoords, shapeNumber, orientation);
    ...
    SDL_SetRenderTarget(renderer, buffer); // all the draw calls go to buffer instead of the sdl backbuffer
    SDL_RenderCopy(renderer, backgroundTexture, NULL, NULL); // copy our own backbuffer in one call
    addPiece(renderer, shapeNumber, orientation, boardCoords, newGrid);
    SDL_SetRenderTarget(renderer, NULL); // we set the renderer back so we can draw to the SDL backbuffer
    SDL_RenderCopy(renderer, buffer, NULL, NULL); // copy our own backbuffer in one call
    SDL_RenderPresent(renderer);      
...
int** renderOnePiece(SDL_Renderer* renderer, SDL_Texture* backgroundTexture, SDL_Texture* buffer, int **grid)
{
    ...
    int incomingPiece[4][4];
    int newGrid[boardWidth][boardHeight];
    ...
    while (running)
    {
        ...
        if(SDL_GetTicks64() >= nextDownMovement)
        {
            ...
            if(nextMove(renderer, backgroundTexture, buffer, grid, boardCoords, shapeNumber, orientation)==false)
            {
                boardCoords = oldBoardCoords;
                std::cout << "should exit here" << std::endl;   
                return updateGrid(grid, oldBoardCoords, shapeNumber, orientation);
            }    
        } 
        else 
        {
